...
public class CInstructor extends AppCompatActivity {
    ...
    private void deleteInstructor(View v) {
        ...
        Cursor cursor = db.rawQuery("SELECT IDINSTRUCTOR FROM INSTRUCTOR WHERE NOMBRE='"+spinstructor.getSelectedItem().toString()+"';",null);
        ...
        if (tmp>0){
            try{
                String [] tmparameters={Integer.toString(tmp)};
                db.delete("HORARIO","INSTRUCTOR=?",tmparameters);
            }catch (Exception e){
    ...
    public void inizaialite(){
        ...
        btnupdate = findViewById(R.id.btnactualizarCI);
        btndelete = findViewById(R.id.btneliminarCI);
