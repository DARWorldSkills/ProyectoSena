...
public class CHorario extends AppCompatActivity {
    ...
    private void updateHorario(View v) {
        ...
        try {
            ...
            horario.setPrograma(db.rawQuery("SELECT IDPROGRA FROM PROGRAMA WHERE NOMBRE ='"+spprograma.getSelectedItem().toString()+"';",null).getInt(0));
    ...
    public void listarDiaHora(){
        ...
        GestorDB gestorDB = new GestorDB(this);
        SQLiteDatabase db = gestorDB.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT IDHORARIO,DIA,HORA FROM HORARIO",null);
        if (cursor.moveToFirst()){
            do {
                Horario horario = new Horario();
